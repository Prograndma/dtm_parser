...
class DTMHeaderReader:
    ...
    def get_inputs(self, dtm):
        ...
        GameID = dtm.read(6)
        IsWiiGame = bool.from_bytes(dtm.read(1))
        ConnectedControllers = int.from_bytes(dtm.read(1))
        IsFromSaveState = bool.from_bytes(dtm.read(1))
        ...
        IsSavedConfig = bool.from_bytes(dtm.read(1))
        UsingIdleSkip = bool.from_bytes(dtm.read(1))
        UsingDualCore = bool.from_bytes(dtm.read(1))
        UsingProgressiveScan = bool.from_bytes(dtm.read(1))
        UsingHLEDSP = bool.from_bytes(dtm.read(1))
        UsingFastDiscSpeed = bool.from_bytes(dtm.read(1))
        CPUCore = int.from_bytes(dtm.read(1))
        IsEFBAccessEnabled = bool.from_bytes(dtm.read(1))
        IsEFBCopiesEnabled = bool.from_bytes(dtm.read(1))
        UsingEFBToTexture = bool.from_bytes(dtm.read(1))
        IsEFBCopyCacheEnabled = bool.from_bytes(dtm.read(1))
        IsEmulatingEFBFormatChanges = bool.from_bytes(dtm.read(1))
        UsingXFB = bool.from_bytes(dtm.read(1))
        UsingRealXFB = bool.from_bytes(dtm.read(1))
        UsingMemoryCard = bool.from_bytes(dtm.read(1))
        UsingClearSaves = bool.from_bytes(dtm.read(1))
        NumBongos = int.from_bytes(dtm.read(1))
        SyncGPU = bool.from_bytes(dtm.read(1))
        UsingNetplay = bool.from_bytes(dtm.read(1))
        PAL60 = bool.from_bytes(dtm.read(1))
